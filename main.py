...
def fetch_weather_with_daily(lat, lon, start_date, end_date, is_past: bool):
    ...
    # ✅ Rain % removed from request entirely
    params = {
        "latitude": lat,
        "longitude": lon,
        "start_date": start_date,
        "end_date": end_date,
        "hourly": "temperature_2m,precipitation,rain,showers,snowfall,windspeed_10m,relativehumidity_2m,cloudcover",
...
def extract_metrics(data, target_dt):
    ...
    return {
        ...
        "Temp (°C)": row.get("temperature_2m"),
        "Wind (km/h)": row.get("windspeed_10m"),
        "Tmin (°C)": tmin,
        "Tmax (°C)": tmax,
        "Rain (mm)": row.get("precipitation"),
        "Snow (mm)": row.get("snowfall")
    }
...
def compute_best_hour(hourly_df):
    ...
    # ✅ removed precipitation_probability penalty
    df["score"] = (
        100
...
if mode == "Quick Forecast":
    ...
    for i in range(5):
        ...
        with cols_time[i]:
            t = st.time_input(
                f"Time {i + 1}",
                value=current_time_paris.time().replace(second=0, microsecond=0),
                key=f"q_time_{i}"
            )
...
# =========================================================
    ...
    if st.button("Generate Detailed Forecast"):
        ...
        display_cols = ["Date", "Time", "Tmin (°C)", "Tmax (°C)", "Temp (°C)",
                        "Rain (mm)", "Snow (mm)", "Wind (km/h)", "Score"]
